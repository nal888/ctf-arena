...
allowed-tools: Bash Read Write Edit Glob Grep Task WebFetch WebSearch mcp__burp__send_http1_request mcp__burp__send_http2_request mcp__burp__create_repeater_tab mcp__burp__create_repeater_tab_http2 mcp__burp__send_to_intruder mcp__burp__get_proxy_http_history mcp__burp__get_proxy_http_history_regex mcp__burp__set_proxy_intercept_state mcp__burp__set_active_editor_contents mcp__burp__get_active_editor_contents mcp__burp__url_encode mcp__burp__url_decode mcp__burp__base64_encode mcp__burp__base64_decode
metadata:
...
## Tooling — Burp + the Repeater-first habit
A hook **enforces** this: `curl` carrying a session cookie to the target is BLOCKED (it's the AI-tell + what tarpits fingerprint) — so authenticated/injection requests must go through the **Burp MCP** (`mcp__burp__*`; Burp runs on proxy :8080, MCP :9876). Use it well:
- **Repeater** (`send_http1_request` / `send_http2_request` / `create_repeater_tab`) — send the injection/auth/SSTI request through Repeater and READ the full response *before* you write any loop. Confirm the vuln + shape the payload here.
- **Intruder** (`send_to_intruder`) — fuzz a parameter across a wordlist/charset.
- **Proxy history** (`get_proxy_http_history[_regex]`) — inspect what the app/browser actually sent; find hidden params/endpoints/tokens/filenames.
- **Encoders** (`url_encode`/`base64_encode` + decode) — payload (de)obfuscation.

**Repeater-first mindset (this avoids the #1 trap):** before scripting a char-by-char / boolean extraction loop, send ONE clean request in Repeater and ask "can I get the whole answer in one response?" Most SQLi/exfil challenges have a **direct dump** path (UNION SELECT into a visible field/report/CSV, error-based, stacked) that returns everything at once — far faster than a boolean loop. Real case (picoCTF ORDER ORDER): the solve was **2 UNION requests in Repeater** dumping the table into the report CSV; an agent that wrote a boolean ORDER-BY extraction loop instead TIMED OUT. Reach for the loop only after Repeater proves no direct dump exists.

Use curl/python for genuine batch work (brute-force, many timed requests). But the *first* contact with any injection point goes through Burp Repeater.
...
- SOURCE given + PINNED dependency/proxy/framework versions (docker-compose, Dockerfile, package.json): WebSearch each exact version for a known CVE *before* hand-rolling — on hard challenges the bug is often the dependency (e.g. Traefik 2.9 `;`->`&`), not your code-reading.
...
- "prepared/parameterized everywhere" + a report/sort/export -> SECOND-ORDER SQLi: payload in a STORED field (username/name) reused in a later unparameterized query (ORDER BY / report SELECT). Watch for a red-herring name (e.g. "ORDER" baiting ORDER BY).
- admin bot + flag in localStorage/a file -> control where the bot navigates (param-injection / open-redirect / proxy-version CVE) -> exfil IN-APP, SIMPLEST first: `url=javascript:` reads localStorage -> POST to a readable endpoint  >  `file://` iframe in the bot screenshot. Bot runs on challenge infra; NO external/ngrok listener.
